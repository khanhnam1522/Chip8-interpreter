...
function drawLoop() {
    ...
    for (let i = 0; i < 9; ++i) {
        for (let j = 0; j < 15; ++j) {
            ctx.fillStyle = "grey";
            ctx.fillRect(i*10, j*10, 1, 10);
            if (i == 8) {
                ctx.fillRect(79, j*10, 1, 10);
            }
        }
    }
...
function convertGridToHex() {
    ...
    document.getElementById("Hex").innerHTML = hexArray;
}
...
function drawPixel(canvas, event) {
    ...
    x = (parseInt(x / 10, 10) * 10)/10;
    y = (parseInt(y / 10, 10) * 10)/10;
